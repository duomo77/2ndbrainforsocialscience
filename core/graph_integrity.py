...
from core.constants import MAX_NODE_NAME_LENGTH
...
class GraphIntegrityEngine:
    ...
    def commit_transaction(self):
        ...
        # 무결성 검증 — C-16: 커밋마다 전체 그래프 O(V+E) DFS를 돌리는 대신
        # 트랜잭션 범위만 검증한다 (전체 검증은 validate()로 별도 호출 가능).
        ok, issue = self.validate_transaction(ops)
        if not ok:
            self.rollback()
            return False, f"Integrity check failed: {issue}"
    ...
    def add_node(
        ...
        # B-12: LLM 출력에서 유래한 초장문 노드명 차단
        if len(title) > MAX_NODE_NAME_LENGTH:
            logger.warning(f"Node rejected (title {len(title)} chars > {MAX_NODE_NAME_LENGTH})")
            return None
    ...
    def add_edge(
        ...
        # B-12: 커밋된 엣지 + 현재 트랜잭션 버퍼의 엣지를 합산한다
        # (버퍼 우회로 캡이 무력화되던 문제를 차단)
        src_id = hashlib.sha256(source_title.strip().lower().encode()).hexdigest()[:16]
        existing = self.store.get_edges_from(src_id)
        buffered = 0
        if self._tx_buffer is not None:
            buffered = sum(
                1 for op_type, obj in self._tx_buffer
                if op_type == "edge" and obj.source == src_id
            )
        if len(existing) + buffered >= MAX_EDGES_PER_NODE:
    ...
    def validate_transaction(self, ops: list) -> tuple[bool, str]:
        """C-16: 트랜잭션 범위 무결성 검증 (전체 그래프 DFS 없음).

        검증 항목: (1) 노드 수 상한, (2) 트랜잭션 엣지의 양 끝 노드 존재,
        (3) 트랜잭션 엣지가 새로 만드는 사이클만 한정 BFS로 검사.
        """
        tx_nodes: dict = {}
        tx_edges: list = []
        for op_type, obj in ops:
            if op_type == "node":
                tx_nodes[obj.node_id] = obj
            else:
                tx_edges.append(obj)

        # 1. 노드 수 상한
        if len(self.store.all_nodes()) + len(tx_nodes) > MAX_NODES_PER_VAULT:
            return False, "node limit exceeded"

        # 2. 엣지 양 끝 노드 존재 (커밋된 노드 + 트랜잭션 노드)
        known_ids = {n.node_id for n in self.store.all_nodes()} | set(tx_nodes)
        for edge in tx_edges:
            if edge.source not in known_ids or edge.target not in known_ids:
                return False, f"edge references missing node ({edge.edge_id})"

        # 3. 트랜잭션 엣지만 사이클 검사 — 인접 맵 오버레이 후 한정 BFS
        if tx_edges:
            base = self.store.adjacency_map()
            adj = {node: list(neighbors) for node, neighbors in base.items()}
            for edge in tx_edges:
                adj.setdefault(edge.source, []).append(edge.target)
            for edge in tx_edges:
                if self._reaches(adj, edge.target, edge.source):
                    return False, f"cycle introduced by edge {edge.edge_id}"
        return True, "ok"

    @staticmethod
    def _reaches(adj: dict, start: str, goal: str, max_visits: int = 5000) -> bool:
        """한정 도달성 검사: start → goal 경로 존재 여부 (방문 노드 수 상한)."""
        if start == goal:
            return True
        visited = set()
