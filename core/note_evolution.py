...
_log = get_logger("core.note_evolution")
...
class NoteEvolutionStore:
    ...
    def get(self, note_id: str) -> Optional[NoteEvolutionRecord]:
        ...
        if d:
            try:
                return NoteEvolutionRecord(**d)
            except (TypeError, KeyError, ValueError) as ex:
                _log.warning(f"Skipping malformed evolution record {note_id}: {ex}")
        return None

    def upsert(self, record: NoteEvolutionRecord):
        record.updated_at = datetime.now(UTC).isoformat()
        # B-11/C-11 수정: 체크섬이 동일한(내용 불변) 재등록은 버전을 올리지 않는다.
        # 재분석 시 중복 register_note 호출로 version이 부풀던 문제를 차단.
        existing_raw = self._records.get(record.note_id)
        if existing_raw is None or existing_raw.get("checksum") != record.checksum:
            record.version += 1
    ...
    def all_records(self) -> list[NoteEvolutionRecord]:
        records = []
        for note_id, d in self._records.items():
            try:
                records.append(NoteEvolutionRecord(**d))
            except (TypeError, KeyError, ValueError) as ex:
                _log.warning(f"Skipping malformed evolution record {note_id}: {ex}")
        return records
