...
class HierarchicalRetriever:
    ...
    def _estimate_abstraction_density(self, content: str) -> float:
        ...
        # WikiLink
        from core.utils.markdown_utils import count_wikilinks
        score += min(count_wikilinks(content) * 0.03, 0.2)
        # 방법론 키워드
...
class RAGEngine:
    ...
    def prepare_context(
        ...
        # NOTE (audit C-13): RAG 컨텍스트는 의도적으로 캐시하지 않음 —
        # 캐싱이 필요하면 여기서부터 키 공간 통일 후 구현할 것.
