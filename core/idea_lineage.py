...
from core.utils.file_utils import atomic_write_json, load_json_store

_log = get_logger("core.idea_lineage")
...
class LineageStore:
    ...
    def _load(self):
        # X-01: corrupt files are quarantined, never silently wiped
        nodes = load_json_store(self._nodes_path, dict, _log)
        self._nodes = nodes if isinstance(nodes, dict) else {}
        branches = load_json_store(self._branches_path, dict, _log)
        self._branches = branches if isinstance(branches, dict) else {}

    def _save(self):
        atomic_write_json(self._nodes_path, self._nodes)
        atomic_write_json(self._branches_path, self._branches)
    ...
    def get_node(self, lineage_id: str) -> Optional[LineageNode]:
        ...
        try:
            ...
            _log.warning(f"Skipping malformed lineage node {lineage_id}: {ex}")
            return None

    def find_by_title(self, title: str) -> Optional[LineageNode]:
        for lineage_id, d in self._nodes.items():
            if d.get("title") == title:
                return self.get_node(lineage_id)
        return None
    ...
    def all_branches(self) -> list[LineageBranch]:
        branches = []
        for branch_id, d in self._branches.items():
            try:
                branches.append(LineageBranch(**d))
            except (TypeError, KeyError, ValueError) as ex:
                _log.warning(f"Skipping malformed lineage branch {branch_id}: {ex}")
        return branches
...
class IdeaLineageEngine:
    ...
    def register_idea(
        ...
        """아이디어 노드 등록 (C-04: 제목 기준 업서트).

        같은 제목은 같은 아이디어다 — 재분석 시 중복 루트 노드를 만드는 대신
        기존 노드를 갱신한다. 자기 자신은 부모가 될 수 없다.
        """
        ...
        if existing is not None:
            ...
            if tags:
                existing.tags = tags
            existing.note_stage = note_stage
            for pt in (parent_titles or []):
                if pt == title:  # C-04: 자기 부모 금지
                    continue
                parent = self.store.find_by_title(pt)
                if (parent is not None
                        and parent.lineage_id != existing.lineage_id
                        and parent.lineage_id not in existing.parent_ids):
                    existing.parent_ids.append(parent.lineage_id)
                    if existing.lineage_id not in parent.child_ids:
                        parent.child_ids.append(existing.lineage_id)
                        self.store.update_node(parent)
            self.store.update_node(existing)
            return existing

        lineage_id = self._make_lineage_id(title, now)

        # 부모 노드 ID 해석 (C-04: 자기 자신 제외)
        parent_ids = []
        for pt in (parent_titles or []):
            if pt == title:
                continue
            parent = self.store.find_by_title(pt)
            if parent and parent.lineage_id != lineage_id:
