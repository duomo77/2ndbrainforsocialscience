...
class AuditTrail:
    """Hash-chained 감사 로그 — append-only JSON Lines (B-15).

    Each event carries ``prev_hash`` (the ``_hash`` of the previous event,
    or the genesis sentinel) so deletion/reordering/insertion of lines is
    detectable via :meth:`verify_chain`. Legacy unchained files written
    before 2026-08-23 will fail verification at the first legacy line —
    this is intentional: chains start fresh at the first chained event.
    """
    ...
    def verify_chain(self) -> tuple[bool, str]:
        """Verify hash-chain integrity. Returns (ok, detail)."""
        if not self._path.exists():
        ...
        for i, line in enumerate(lines):
            ...
            if entry.get("prev_hash") != prev:
                return False, f"line {i}: broken chain link"
            stored = entry.pop("_hash", None)
            recomputed = hashlib.sha256(
                json.dumps(entry, sort_keys=True).encode()
            ).hexdigest()[:12]
            if stored != recomputed:
                return False, f"line {i}: content hash mismatch"
            prev = stored
    ...
    def _last_hash(self) -> str:
        ...
        for line in reversed(lines):
            if not line.strip():
                continue
            try:
                return json.loads(line).get("_hash", self.GENESIS_HASH)
            except json.JSONDecodeError:
                continue
        return self.GENESIS_HASH
...
class SecurityGate:
    ...
    def validate(
        ...
        # 5. 차단 여부 결정 (fail-closed: CRITICAL 위협은 점수와 무관하게 항상 차단)
        has_critical = any(
            t.threat_level == ThreatLevel.CRITICAL.value for t in threats
        )
        is_blocked = trust_score < block_threshold or has_critical
        if is_blocked:
    ...
    def validate_input(self, content: str, source: str = "unknown") -> "ValidationResult":
        """validate() 의 별칭 — 모든 입력 검증의 단일 진입점."""
        return self.validate(content, source)

    def validate_llm_output(self, markdown: str) -> "ValidationResult":
        """B-04: LLM 출력 검증 경계.

        모델 출력은 어떤 저장소 변형(그래프/메모리/볼트)보다 먼저 이 게이트를
        통과해야 한다. 입력과 동일한 위협 패턴 스캔 + 위키링크 홍수 제한이
        적용되며, 닫히지 않은 YAML frontmatter는 복구된다.
        """
        content = markdown or ""
        if content.startswith("---") and "\n---" not in content[3:]:
            content = content.rstrip() + "\n---\n"
        return self.validate(content, "llm_output")
