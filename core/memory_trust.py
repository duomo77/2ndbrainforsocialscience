...
# NOTE (audit C-09): RETRIEVAL_BOOST removed — repetition must not raise trust.
...
class MemoryStore:
    ...
    def _load(self):
        # X-01: corrupt files are quarantined; a single malformed record
        # must never abort the entire trust history
        data = load_json_store(self._path, list, logger)
        ...
        for r in data:
            try:
                ...
                logger.warning(
                    f"Skipping malformed memory record "
                    f"{r.get('memory_id', '?') if isinstance(r, dict) else '?'}: {ex}"
                )

    def save(self):
        atomic_write_json(self._path, [asdict(r) for r in self._records.values()])
...
class MemoryTrustScorer:
    ...
    def apply_decay(self, record: MemoryRecord) -> float:
        ...
        days_elapsed = (datetime.now(UTC) - created).days
        if days_elapsed <= 0:
...
class MemoryTrustEngine:
    ...
    def store_memory(
        ...
        if existing:
            # C-09 수정: 반복 저장으로 신뢰도가 상승하지 않는다.
            # (구 RETRIEVAL_BOOST 제거 — 반복은 검증이 아니며,
            #  환각 강화 루프를 구조적으로 차단한다.)
            existing.access_count += 1
            existing.last_accessed = datetime.now(UTC).isoformat()
    ...
    def retrieve(
        ...
        def rank(r: MemoryRecord) -> float:
            recency = 1.0 / max(
                (datetime.now(UTC) - datetime.fromisoformat(r.last_accessed)).days + 1, 1
            )
