...
_log = get_logger("core.math_ontology")
...
class MathOntologyStore:
    ...
    def _load(self):
        ...
        # the quarantined copy preserves user-linked objects for recovery
        data = load_json_store(self._path, dict, _log)
        self._objects = data if isinstance(data, dict) else {}

    def _save(self):
        atomic_write_json(self._path, self._objects)
    ...
    def _reconstruct(self, object_id: str, d: dict) -> Optional[MathObject]:
        try:
            return MathObject(**d)
        except (TypeError, KeyError, ValueError) as ex:
            _log.warning(f"Skipping malformed math ontology object {object_id}: {ex}")
            return None

    def get(self, name: str) -> Optional[MathObject]:
        for object_id, d in self._objects.items():
            if str(d.get("name", "")).lower() == name.lower():
                return self._reconstruct(object_id, d)
        return None
    ...
    def search(self, query: str) -> list[MathObject]:
        ...
        for object_id, d in self._objects.items():
            if (q in str(d.get("name", "")).lower() or
                ...
                obj = self._reconstruct(object_id, d)
                if obj is not None:
                    results.append(obj)
        return results
    ...
    def link_paper(self, object_name: str, paper_title: str):
        ...
        for obj_id, d in self._objects.items():
            if str(d.get("name", "")).lower() == object_name.lower():
