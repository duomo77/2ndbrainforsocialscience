...
from core.constants import MAX_NODE_NAME_LENGTH, MAX_WIKILINKS_PER_NOTE
...
class SemanticMarkdownExtractor:
    ...
    def extract(self, title: str, markdown: str, source_type: str, source_ref: str = "") -> GraphMutation:
        ...
        # B-04: LLM-출발 변형은 상한 내에서만 노드를 생성한다
        link_budget = MAX_WIKILINKS_PER_NOTE

        for raw_line in (markdown or "").splitlines():
            heading = self._heading.match(raw_line)
            if heading:
                section = heading.group(1).casefold()
            links = _unique(extract_wikilink_targets(raw_line))
            if link_budget <= 0:
                links = []
            else:
                links = links[:link_budget]
                link_budget -= len(links)
            ...
            if question and len(question.group(1)) <= MAX_NODE_NAME_LENGTH \
                    and ("question" in section or not links):
