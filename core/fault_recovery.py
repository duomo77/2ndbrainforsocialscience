...
class ErrorCategory(Enum):
    ...
    API_NOT_FOUND   = "api_not_found"    # 모델/엔드포인트 없음 (404) — 재시도 불가
    API_RATE_LIMIT  = "api_rate_limit"   # 속도 제한 (429)
    API_TIMEOUT     = "api_timeout"      # 타임아웃
    API_SERVER      = "api_server"       # 서버 오류 (500, 503)
    API_CONNECTION  = "api_connection"   # 연결 실패
    PARSE_ERROR     = "parse_error"      # 입력 파싱 오류 — 재시도 불가
...
def classify_error(error: Exception) -> ErrorCategory:
    """F-11: 타입/상태 기반 우선 분류 — 문자열 휴리스틱은 최후 수단.

    숫자 부분문자열 매칭("500"이 "1500ms"에 걸리던 문제)을 제거하고,
    SDK 예외가 운반하는 status_code / 예외 클래스명을 먼저 사용한다.
    """
    ...
    if status is not None:
        ...
        if status == 404:
            return ErrorCategory.API_NOT_FOUND
        if status == 429:
    ...
    if "timeout" in msg or "timed out" in msg:
        return ErrorCategory.API_TIMEOUT
    if "server error" in msg or "internal server" in msg:
        return ErrorCategory.API_SERVER
    if "parse" in msg or "decode" in msg:
...
class SafeMode:
    ...
    # B-05b: @@SENTINEL@@ 자리표시자 — str.format은 사용자 제목/내용의
    # { } 문자열에서 KeyError/ValueError를 일으키므로 사용하지 않는다.
    SAFE_MODE_TEMPLATE = """---
title: "@@TITLE@@"
authors: []
year: @@YEAR@@
...
"""
    ...
    def generate_fallback_note(
        ...
        replacements = {
            "@@TITLE@@":           str(title),
            "@@YEAR@@":            str(year or datetime.now(UTC).year),
            "@@CONTENT_PREVIEW@@": content[:500].replace("`", "'"),
            "@@INPUT_TYPE@@":      str(input_type),
            "@@TIMESTAMP@@":       datetime.now(UTC).strftime("%Y-%m-%d %H:%M UTC"),
            "@@ERROR_REASON@@":    str(error_reason),
        }
        note = self.SAFE_MODE_TEMPLATE
        for sentinel, value in replacements.items():
            note = note.replace(sentinel, value)
        return note
        ...
        **kwargs,
        ...
        for attempt in range(max_retries):
            try:
                ...
                # 재시도 불가 오류 (F-11: 영구 오류에 백오프 시간 낭비 금지)
                if category in (
                    ErrorCategory.API_AUTH,
                    ErrorCategory.API_NOT_FOUND,
                    ErrorCategory.PARSE_ERROR,
                    ErrorCategory.PROMPT_INJECT,
                ):
                    break
