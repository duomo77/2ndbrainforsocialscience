...
_log = get_logger("core.research_tension")
...
class KnowledgeGraphDB:
    ...
    def _load(self):
        ...
        edges = load_json_store(self._edges_path, dict, _log)
        self._edges = edges if isinstance(edges, dict) else {}

    def _save(self):
        atomic_write_json(self._nodes_path, self._nodes)
        atomic_write_json(self._edges_path, self._edges)
...
class ResearchTensionEngine:
    ...
    def _load(self):
        # X-01: corrupt files are quarantined, never silently wiped
        data = load_json_store(self._alerts_path, dict, _log)
        self._alerts = data if isinstance(data, dict) else {}

    def _save(self):
        atomic_write_json(self._alerts_path, self._alerts)
