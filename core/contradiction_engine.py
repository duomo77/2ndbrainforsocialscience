...
_log = get_logger("core.contradiction_engine")
...
class ContradictionStore:
    ...
    def _load(self):
        ...
        data = load_json_store(self._path, dict, _log)
        self._edges = data if isinstance(data, dict) else {}

    def _save(self):
        atomic_write_json(self._path, self._edges)

    def add_edge(self, edge: ContradictionEdge):
        self._edges[edge.contradiction_id] = asdict(edge)
        self._save()

    def _reconstruct(self, contradiction_id: str, d: dict) -> Optional[ContradictionEdge]:
        try:
            return ContradictionEdge(**d)
        except (TypeError, KeyError, ValueError) as ex:
            _log.warning(f"Skipping malformed contradiction {contradiction_id}: {ex}")
            return None

    def get_all(self) -> list[ContradictionEdge]:
        edges = []
        for contradiction_id, d in self._edges.items():
            edge = self._reconstruct(contradiction_id, d)
            if edge is not None:
                edges.append(edge)
        return edges
    ...
    def get_for_note(self, note_title: str) -> list[ContradictionEdge]:
        return [
            edge for edge in self.get_all()
            if edge.source_note == note_title or edge.target_note == note_title
        ]

    def get_unresolved(self) -> list[ContradictionEdge]:
        return [edge for edge in self.get_all() if not edge.resolved]
