...
Persistence contract (X-01): every store writes atomically
(tmp + fsync + replace) and corrupt files are quarantined —
never silently reset to empty.
"""
...
def load_profile() -> dict:
    _ensure_dir()
    profile = load_json_store(PROFILE_FILE, lambda: DEFAULT_PROFILE.copy(), _log)
...
def load_concepts() -> dict:
    ...
    _ensure_dir()
    concepts = load_json_store(CONCEPTS_FILE, dict, _log)
    return concepts if isinstance(concepts, dict) else {}

def save_concepts(concepts: dict):
    _ensure_dir()
    atomic_write_json(CONCEPTS_FILE, concepts)
...
def load_questions() -> list:
    _ensure_dir()
    questions = load_json_store(QUESTIONS_FILE, list, _log)
...
def load_graph() -> dict:
    ...
    _ensure_dir()
    graph = load_json_store(GRAPH_FILE, dict, _log)
