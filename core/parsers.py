...
B-14 계약 (2026-08-23):
  - `MAX_INPUT_FILE_BYTES` 초과 파일은 파싱 전에 거부된다 (DoS 게이트).
  - 파싱 오류는 콘텐츠로 반환되지 않고 `meta["parse_error"]` 신호로 전달된다.
    오류 문자열이 LLM 프롬프트나 볼트로 흘러가지 않는다.
"""
...
from core.constants import MAX_INPUT_FILE_BYTES


def _file_gate_error(path: str) -> Optional[str]:
    """B-14: 읽기 불가능하거나 한도를 넘는 파일이면 사유를 반환."""
    try:
    ...
    if size > MAX_INPUT_FILE_BYTES:
        ...
        return f"파일이 너무 큽니다 ({size / (1024 * 1024):.1f} MB > {limit_mb:.0f} MB 한도)"
    return None

# ── PDF Parser ────────────────────────────────────────────────────────────────
def parse_pdf(path: str, max_chars: int = 80000) -> tuple[str, dict]:
    """PDF에서 텍스트 추출. (text, metadata) 반환."""
    gate = _file_gate_error(path)
    if gate:
        return "", {"parse_error": gate}
    try:
        import fitz  # PyMuPDF
        doc = fitz.open(path)
        ...
        return "", {"parse_error": "PyMuPDF not installed — pip install PyMuPDF"}
    except Exception as e:
        return "", {"parse_error": f"PDF parse error: {e}"}

# ── Text / Markdown Parser ────────────────────────────────────────────────────
def parse_text(path: str, max_chars: int = 80000) -> str:
    """텍스트/마크다운 파일 읽기."""
    if _file_gate_error(path):
        return ""
    try:
...
"""
        ...
        return "", {"parse_error": "pandas not installed — pip install pandas openpyxl"}
    except Exception as e:
        return "", {"parse_error": f"Dataset parse error: {e}"}
...
def parse_transcript(path: str, max_chars: int = 80000, raw_text: str = "") -> tuple[str, dict]:
    ...
    try:
        ...
        return "", {"parse_error": f"Transcript parse error: {e}"}
...
def parse_code(path: str) -> tuple[str, dict]:
    ...
    try:
        ...
        return "", {"parse_error": f"Code parse error: {e}"}
