...
# NOTE: get_embedding_governance / get_memory_tier_manager removed (audit F-06):
# they referenced factories that never existed. core.embedding_gov now provides
# SemanticDeduplicator only; governance engines must be re-added with real
# implementations before getters are reintroduced.
...
def get_scientific_context_engine() -> Optional[Any]:
    """Scientific Knowledge Expansion 엔진 (literature 패키지, EPIC 09)."""
    def _f():
        from literature.context.engine import ScientificContextEngine
        return ScientificContextEngine()
    return _load("scientific_context", _f)
