...
def _restrict_secret_permissions(path: Path) -> None:
    """B-08: config files contain API keys — owner-only permissions.

    No-op on platforms without POSIX permission bits (best effort).
    """
    try:
        os.chmod(path, 0o600)
    except OSError:
