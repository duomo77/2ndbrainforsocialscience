...
class MainWindow(QMainWindow):
    ...
    def _build_toolbar(self):
        ...
        self._analyze_action = a1  # F-13: 버튼과 함께 분석 액션도 비활성화
        a_stop = QAction("⏹ 정지", self); a_stop.triggered.connect(self._stop_analysis); tb.addAction(a_stop)
        tb.addSeparator()
    ...
    def _start_analysis(self):
        # F-13: single-flight — 이전 분석이 실행 중이면 중복 시작을 막는다
        if self._worker is not None and self._worker.isRunning():
            QMessageBox.information(self, "분석 진행 중",
                                    "이미 분석이 실행 중입니다. 완료 후 다시 시도하세요.")
            return
        ...
        self._analyze_action.setEnabled(False)  # F-13: 툴바 액션도 함께 잠금
        ...
        self._worker.finished.connect(self._worker.deleteLater)  # F-13: 누출 방지
        self._worker.start()
    ...
    def _on_done(self, result):
        ...
        self._analyze_action.setEnabled(True)  # F-13
    ...
    def _stop_analysis(self):
        """B-06: 협동 취소 요청 — 다음 체크포인트에서 중단된다."""
        if self._worker is not None and self._worker.isRunning():
            self._worker.cancel()
            self._set_status("⏹ 취소 요청됨 — 다음 체크포인트에서 정지합니다")

    def closeEvent(self, event):
        # F-13: 실행 중인 워커를 방치하지 않음 — 저장소 쓰는 중 중단 방지 (최대 2초 대기)
        if self._worker is not None and self._worker.isRunning():
